...
const TopBar = () => {
  ...
  useEffect(() => {
    try {
      ...
      setUserName(`${user.prenom || ''} ${user.nom || ''}`.trim() || 'Utilisateur');
    } catch (error) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 lg:left-[20vw] z-30 bg-white border-b border-gray-200 shadow-sm">
      <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
        {/* Gauche : Titre ou info */}
        <div className="flex items-center gap-3">
          <span className="material-icons text-purple-600 text-2xl sm:text-3xl">wb_sunny</span>
          <div>
        ...
        <div className="relative">
          ...
          >
            ...
            {/* Badge compteur */}
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center shadow-lg animate-pulse">
                ...
                {notifications.length > 0 && (
                  <div className="p-3 border-t border-gray-200 bg-gray-50">
                    <button
                      ...
                      className="w-full text-center text-xs sm:text-sm text-blue-600 hover:text-blue-700 font-medium transition py-2"
                    >
